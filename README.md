...
answers its own JSON `problem+json` 404 rather than the UI's HTML page
(`internal/api/router.go`'s `apiNotFoundHandler`). See
...
# Individual steps -- each is now a real prerequisite of the next (so
# `make -j` can't interleave them): web-check depends on web-types,
# web-build depends on web-check, web depends on web-build. Invoking any
# one of them also runs everything it depends on.
make web-types  # openapi-typescript -> web/assets/ts/gen/types.d.ts
make web-check  # web-types, then tsc --noEmit (skipped with a notice if npm is absent)
