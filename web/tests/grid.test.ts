...
// logic. grid.ts's draft-mode verdict helper (draftAriaPrefix) is covered
// in draft.test.ts alongside the rest of the draft model it renders. The
// geometry section of grid.ts never touches the DOM, so no stubs are
// needed; all instants are built with local-time Date constructors so
