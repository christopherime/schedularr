...
- **Day tabs** (`SUN 30 … SAT 05`) — navigate within the already-loaded window; tabs never re-plan. The plan window is `[fetch time, fetch time + DAYS×24h)`, so any load after midnight spills into a trailing partial calendar day that gets its own tab — up to DAYS+1 tabs (a 7-day plan loaded at 19:00 shows 8). Today's tab carries a small accent dot. A slot that crosses midnight is cut at the day edge with a dashed edge and continues on the neighboring day.
