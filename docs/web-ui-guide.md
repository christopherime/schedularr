...
1. Open the UI (`http://<host>:8484/` by default). With no token stored, the UI probes `GET /api/v1/status` first: a `401` opens the **Arm API Token** panel automatically, while a deployment running with `api.insecure_no_auth: true` answers without a token and never sees the panel at all — the panel only ever appears when the API actually demands a token.
...
Every page carries a print stylesheet: the light palette always (the dark palette is screen-only — a printout is the instrument's calibration sheet on paper), and everything that exists to be clicked comes off the sheet — nav, telemetry, the token trigger, filters, pagers, tabs, action buttons and the deletion desk — leaving the readings. Tables drop their actions column, rows and run cards never split across pages, and wide tables clip at the sheet edge rather than spilling onto a second horizontal page. The Guide prints the sheet it was showing, clipped to the page; the History page prints as a station log.
