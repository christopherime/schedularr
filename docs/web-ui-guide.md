...
There is deliberately no LIVE/POLL link legend yet — that arrives with the SSE live layer (v0.5.6), and the strip does not pretend to be live before then.
...
The old dashboard left the navigation in v0.5.1: its status readouts live in the bezel telemetry strip on every page, and the Guide took home. The route survives **unlinked** at `/dashboard/` only for its Recent History table — reads `GET /api/v1/history?days=7` with channel legend plates — and is deleted outright when the Log page absorbs history in v0.5.5. Don't bookmark it.
...
Preview a schedule, then apply it — a dry-run/apply pair backed by `POST /api/v1/generate` and `POST /api/v1/apply`, the same `GenerateRequest` body (`days`, optional `channel_id`) as the CLI's `generate`/`generate --apply`. A chronological list per channel; this page remains the apply surface until v0.5.4 folds preview/apply into the Guide as its draft mode (and then it is deleted).
