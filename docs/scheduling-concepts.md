...
- A `PATCH /state/series/{show_title}` (manual cursor reset) or a block edit/delete invalidates every not-yet-*finished* occurrence snapshot for the affected block(s) — including one that's currently on air, not just occurrences that haven't started yet — so the change takes effect on the very next apply instead of being shadowed until every already-snapshotted occurrence ages out of the window on its own. The persisted cursor itself only ever advances by what actually aired (never by silently re-picking content against a since-edited spec), so a manual PATCH always takes effect immediately and stays in effect — it's never overwritten back to whatever an on-air occurrence's own already-aired content would otherwise imply. See the [API Reference](api-reference.md#series-state).
