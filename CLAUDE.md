...
│   ├── index.html                   # The Guide ("/"): EPG grid + draft & apply
│   ├── 404.html                     # Styled not-found page, composed through baseof
│   ├── blocks/list.html              # Blocks ("/blocks/")
│   └── history/list.html              # History ("/history/"): TRACKED / AS-RUN / RUNS panes
├── content/                       # Hugo section front matter (blocks/, history/ _index.md)
├── assets/
│   ├── css/main.css                 # Hand-written, one file, CSS custom properties (light/dark)
│   ├── ts/                          # runtime/ (api.ts, token.ts, grid.ts, draft.ts, …); gen/types.d.ts generated by `make web-types`
│   │   └── pages/                     # guide.ts, blocks.ts, history.ts
...
**Retention is per table.** `maintenance.history_retention` (`168h`),
`maintenance.snapshot_retention` (`168h`), and
`maintenance.apply_run_retention` (`2160h`, 90 days) each prune their own
table on their own clock. `history_retention` also governs the engine's
in-memory recency-dedup window and bounds how far back `GET
/history?days=N` can return data; `apply_run_retention` bounds `GET
/applies?days=N` the same way. Setting `snapshot_retention` longer than
`history_retention` keeps snapshot rows that can never be replayed,
because the `schedule_history` rows they depend on are already gone.
