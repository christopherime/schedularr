...
**Current version:** v0.5.6 (2026-09-08). Latest release put draft & apply on the Guide and deleted the standalone preview page.
...
│   │   ├── runtime/              # api.ts, token.ts, shell.ts, grid.ts, draft.ts, etc.
│   │   ├── pages/                # guide.ts, dashboard.ts, blocks.ts, series.ts
